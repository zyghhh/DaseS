...
MCP 服务随后端 FastAPI 自动启动，无需额外操作。只需在 CLI 工具中配置 HTTP 端点即可。

**Claude Code** — 在 `.claude/settings.local.json`（项目级）或 `~/.claude/settings.json`（全局）中：
...
**Gemini CLI** — 在 `.gemini/settings.json`（项目级）中：
...
配置完成后，在 Claude Code 中即可使用 Slash Command：
```
